...
for cluster in "${CLUSTER_LIST[@]}"; do
  ...
  if cluster_exists "${cluster}"; then
    if [[ "${RECREATE}" == "true" ]]; then
      ...
      controller_name="$(cluster_controller_name "${cluster}")"
      if [[ "$(docker inspect -f '{{.State.Running}}' "${controller_name}" 2>/dev/null)" != "true" ]]; then
        echo "Container '${controller_name}' is stopped. Starting..."
        if ! docker start "${controller_name}" >/dev/null 2>&1; then
          echo "Container '${controller_name}' could not be restarted (stale bind-mounts or config change). Recreating cluster '${cluster}'..."
          cluster_delete_runtime "${cluster}"
        else
          if ! wait_for_controller "${cluster}"; then
            echo "Cluster '${cluster}' failed to become ready after restart." >&2
            exit 1
          fi
          configure_container_mounts "${controller_name}"
          write_kubeconfig "${cluster}"
